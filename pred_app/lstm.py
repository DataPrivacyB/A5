...
def ltm_prediction(se, stock_symbol):

    def fetch_stock_data(se, stock_symbol):
        """fetch stock data"""
        df = pd.read_csv("C:\\Users\\Akshay Bali\\Desktop\\A5\\pred_app\\DataSets\\Nifty4YearsData\\{}.csv".format(stock_symbol))
        print(df.head())
        # yf.pdr_override()
    ...
    h = lstm_model.fit(x_totrain, y_totrain, epochs=1, batch_size=1, verbose=2)
    print(".history['loss']:",h.history['loss'])
    # print("history.history['accuracy']:",h.history['accuracy'])
    ...
    print("Type of test:", type(tostore_test_result))
    tostore_test_result = np.reshape(tostore_test_result,
                                     (tostore_test_result.shape[0], tostore_test_result.shape[1], 1))
    print("Type of test1:", type(tostore_test_result))
    myclosing_priceresult = lstm_model.predict(tostore_test_result)
    print("Type of predict:",type(myclosing_priceresult))
    myclosing_priceresult = scalerdata.inverse_transform(myclosing_priceresult)
    print("Type of predict1:", type(myclosing_priceresult))
    # Predict = model.predict(X_test)
    # testScore = math.sqrt(mean_squared_error(y_test, Predict))
    # print('Test Score: %.2f RMSE' % (testScore))
    # plt.plot(tostore_test_result)
    ...
    tostore_test_result = np.reshape(tostore_test_result,(tostore_test_result.shape[0],1,tostore_test_result.shape[1]))
    test_df = pd.DataFrame(tostore_test_result, columns=['Close'], index=datelist)
